...
| M-04 | `warehouses/quote/hooks.ts` | Al aumentar la cantidad de una línea, si no hay unidades disponibles suficientes **se acuñan unidades nuevas**. | Una cotización puede crear inventario físico que no existe. | **PRESTACIÓN** (decidido 2026-08-19): el almacén trae de fuera lo que no tiene, así que comprometerlo es negocio normal. Se exige autorización explícita en el mostrador —la tienda pública sigue rechazándolo, porque ahí no hay a quién pedírsela—, la unidad queda marcada, y la llegada se confirma aparte sin borrar la marca |
