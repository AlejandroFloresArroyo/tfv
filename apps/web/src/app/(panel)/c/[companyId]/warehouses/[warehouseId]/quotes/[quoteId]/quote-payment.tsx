      ...
      <Panel className="space-y-5 p-5">
        ...
        {form.spreadFees && form.fixedPrice.trim() !== "" ? (
          <p className="inline-flex items-center gap-1.5 text-body3 text-content-muted">
            <TriangleAlert className="size-4 shrink-0 text-tinta-cuida" aria-hidden="true" />
            {t("spreadFeesMoot")}
          </p>
        ) : null}
 ...
 *
 * El aviso y el visto van con **la pareja cruda de la paleta**, no con `text-tinta-cuida` y
 * `text-tinta-firme`: esos dos nombres no existen. El sistema de diseño declara `danger` y ninguno de
...
export function SaveState({
  ...
  if (incomplete)
    return (
      <span className="inline-flex items-center gap-1 text-body3 text-tinta-cuida">
  ...
  return (
    <span className="inline-flex items-center gap-1 text-body3 text-content-muted">
      <Check className="size-4 text-tinta-firme" aria-hidden="true" />
      {t("autosaved")}
    </span>
  )
}
